...
class EditPreferSameSexScreen extends StatefulWidget {
  @override
  _EditPreferSameSexScreenState createState() =>
      _EditPreferSameSexScreenState();
}

class _EditPreferSameSexScreenState extends State<EditPreferSameSexScreen> {
  // 同性向け好み条件の変数
  ...
  Future<void> _fetchUserDetail() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          if (data['preferredSameSexHeightType'] != null) {
            _selectedPreferredHeightType = HeightType.values.firstWhere(
              (ht) => ht.value == data['preferredSameSexHeightType'],
              orElse: () => HeightType.UNDER_170,
            );
          }
          if (data['preferredSameSexBodyType'] != null) {
            _selectedPreferredBodyType = BodyType.values.firstWhere(
              (bt) => bt.value == data['preferredSameSexBodyType'],
              orElse: () => BodyType.NORMAL,
            );
          }
          if (data['preferredSameSexActivityType'] != null) {
            _selectedPreferredActivityType = ActivityType.values.firstWhere(
              (at) => at.value == data['preferredSameSexActivityType'],
              orElse: () => ActivityType.INDOOR,
            );
          }
          if (data['preferredSameSexIncome'] != null) {
            _selectedPreferredIncome = Income.values.firstWhere(
              (inc) => inc.value == data['preferredSameSexIncome'],
              orElse: () => Income.UNDER_100,
            );
          }
          _preferredFoodController.text = data['preferredSameSexFood'] ?? '';
        });
      } else {
        throw Exception('同性の好み条件の取得に失敗しました');
      }
    } catch (e) {
      print('エラーが発生しました: $e');
      showAnimatedSnackBar(
        context,
        message: '同性の好み条件の取得に失敗しました',
  ...
  Future<void> _saveUserData() async {
    try {
      ...
      final response = await http.patch(
        ...
        body: json.encode({
          'preferredSameSexHeightType': _selectedPreferredHeightType?.value,
          'preferredSameSexBodyType': _selectedPreferredBodyType?.value,
          'preferredSameSexActivityType': _selectedPreferredActivityType?.value,
          'preferredSameSexIncome': _selectedPreferredIncome?.value,
          'preferredSameSexFood': _preferredFoodController.text,
        }),
      );
      if (response.statusCode == 200) {
        showAnimatedSnackBar(
